...
import { ShieldCheck, Target, Cpu, Lock, Settings2, BarChart3 } from 'lucide-react';
            ...
            <section className="py-24 lg:py-32">
                <div className="max-w-7xl mx-auto px-6 lg:px-8">
                    <div className="grid lg:grid-cols-12 gap-16 lg:gap-24">
                        ...
                        {/* Left Column: Icons & Visual Cues */}
                        <div className="lg:col-span-4 space-y-12">
                            <div className="aggressive-border p-8 bg-slate-50 border border-slate-100">
                                <ShieldCheck className="text-blue-700 w-10 h-10 mb-6" />
                                <h3 className="text-slate-900 font-black uppercase text-xs tracking-widest mb-2">Confiabilidade</h3>
                                <p className="text-slate-500 text-[10px] font-bold uppercase tracking-tight leading-relaxed">
                                    Engenharia aplicada à redução de incertezas operacionais.
                                </p>
                            </div>

                            <div className="space-y-8 pl-4">
                                {[
                                    { icon: <Cpu size={20} />, label: "Tecnologia Própria" },
                                    { icon: <Lock size={20} />, label: "Segurança de Dados" },
                                    { icon: <BarChart3 size={20} />, label: "Precisão em KPIs" },
                                    { icon: <Settings2 size={20} />, label: "Aderência Operacional" }
                                ].map((item, idx) => (
                                    <div key={idx} className="flex items-center space-x-4 group">
                                        <div className="text-blue-700 transition-transform group-hover:scale-110">
                                            {item.icon}
                                        </div>
                                        <span className="text-slate-900 font-black uppercase text-[10px] tracking-[0.2em]">
                                            {item.label}
                                        </span>
                                    </div>
                                ))}
